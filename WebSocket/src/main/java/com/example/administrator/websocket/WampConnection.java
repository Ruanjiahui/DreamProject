...
public class WampConnection extends WebSocketConnection implements Wamp {
    ...
    public void connect(String wsUri, Wamp.ConnectionHandler sessionHandler) {
        ...
        options.setMaxFramePayloadSize(4096 * 1024);
            ...
            connect(wsUri, new String[]{"wamp"}, new WebSocketConnectionHandler() {
                ...
                /*            @Override
                            public void onTextMessage(String payload){
                               if (mSessionHandler != null) {
                                  mSessionHandler.onTextMessage(payload);
                               } else {
                                  if (DEBUG) Log.d(TAG, "could not receive TextMessage .. handler already NULL");
                               }
                            }
                            public void onRawTextMessage(byte[] payload){
                               if (mSessionHandler != null) {
                                  mSessionHandler.onRawTextMessage(payload);
                               } else {
                                  if (DEBUG) Log.d(TAG, "could not receive onRawTextMessage .. handler already NULL");
                               }
                            }
                */
                @Override
                public void onBinaryMessage(byte[] payload) {
                    if (mSessionHandler != null) {
                        mSessionHandler.onBinaryMessage(payload);
                    } else {
                        if (DEBUG)
                            Log.d(TAG, "could not receive BinaryMessage .. handler already NULL");
                    }
                }
    ...
    public void sendBinaryMessage(byte[] payload) {
        super.mWriter.forward(new WampMessage.BinaryMessage(payload));
    }
