...
public class WampWriter extends WebSocketWriter {
   ...
   private void sendBinaryMessage(WampMessage.BinaryMessage message) throws IOException, WebSocketException {
...
//      }
   ...
   protected void processAppMessage(Object msg) throws WebSocketException, IOException {
      ...
      try {
         ...
         if (msg instanceof WampMessage.BinaryMessage){
//            if (msg.mPayload.length > mOptions.getMaxMessagePayloadSize()) {
//               throw new WebSocketException("message payload exceeds payload limit");
//            }
            //sendFrame(2, true, mPayload.getByteArray());

            sendBinaryMessage((WampMessage.BinaryMessage) msg);
         }
         else
         {
